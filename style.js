...
oxfordFlippedApp.config.nameActivityCredits = 'Créditos';
...
oxfordFlippedApp.text = {
	...
	gamificacion_monedas_insuficientes: textweb('style_flipped_nocoins'),
...
oxfordFlippedApp.getMainLockState = function(chapterLockStatus) {
	// 0 = Popup License --> Teacher + Lock 2 (statusLock2)
	// 1 = Popup License --> Lock 8, No license, Student (statusLock1)
	// 2 = Popup Teacher --> Stud + License + No Teacher access (8) (statusLock1)
	// 3 = Access
...
oxfordFlippedApp.hashDistributor = function(currentHash,data,updateHash) {
	...
	if (currentHash === oxfordFlippedApp.config.tree[0].hash) { //Home
		...
		hashDistributorTimeout = setTimeout(function() {oxfordFlippedApp.loadGradebook(updateHash, false)}, timeToWait);
	...
	if (oxfordFlippedApp.config.firstTime) {
		...
		$.each(data.units[0].subunits, function(i, subunit){
			var unitTitle = subunit.title,
					...
					unitIsCredits = (subunit.title === oxfordFlippedApp.config.nameActivityCredits);
			if (unitIsInfo && oxfordFlippedApp.config.isTeacher) {
				$('.oxfl-js-open-info').show().attr('onclick', subunit.onclickTitle);
			} else if (unitIsCredits) {
				$('.oxfl-js-open-credits').show().attr('onclick', subunit.onclickTitle);
			}
		});
	...
	$.each(chapters, function(i, chapter){
		...
		if (!chapterIsMarketplace && !chapterIsOnlyVisibleTeacher) {
			...
			if (!chapterIsChallenge) {
				...
				var chapterPopoverText = oxfordFlippedApp.text.oxfordFlipped_no_access_alert;
				...
				if (resource.tag === oxfordFlippedApp.config.tagMarketplace) {
					...
					if ((type === oxfordFlippedApp.config.marketplaceType1 && isGame) || (type === oxfordFlippedApp.config.marketplaceType2 && !isGame)) {
						...
						var resourceTitle = resource.title,
								...
								resourceStateClass = (oxfordFlippedApp.config.isStudent && resourceStateNew && resourceValue !== 0) ? 'oxfl-resource-locked' : '',
...
oxfordFlippedApp.loadGradebook = function(updateHash, justUpdate) {
	...
	$.each(oxfordFlippedApp.bookData.units, function(i, unit){
		if (i !== oxfordFlippedApp.config.ConfigActivityIndex) {
			var subunits = unit.subunits,
					...
					resourceListItem.innerHTML = '<article class="oxfl-resource"> <a href="javascript:void(0)" class="oxfl-js-open-resource-unit" data-resource-unit-number="'+number+'"><header class="oxfl-resource-header"> <h2 class="oxfl-title2">'+title+'</h2><h3 class="oxfl-title4">'+description+'</h3></header> <div class="oxfl-resource-image-wrapper"><div class="oxfl-resource-image-cover"></div> <div class="oxfl-resource-image-wrapper-img">'+image+'</div> </div> </a> </article>';
					resourceList.appendChild(resourceListItem);
		}
	});
...
oxfordFlippedApp.closeIframe = function() {
	...
	if (blink.isApp && !blink.isOfflinePC) {
		// Comprobamos si estamos en un challenge.
		if (this.contentZone){
			blink.activity.currentStyle.exitSequencing();
		} else {
			blink.activity.currentStyle.exitChallenge();
		}
...
oxfordFlippedApp.activityTestSlides = function(contentZoneIndex) {
	...
	var customSound  = '<div style="display: none"><audio id="audio0"><source src="' + oxfordFlippedApp.sounds[0] + '" type="audio/mpeg"></audio></div>';
			customSound += '<div style="display: none"><audio id="audio1"><source src="' + oxfordFlippedApp.sounds[1] + '" type="audio/mpeg"></audio></div>';
			customSound += '<div style="display: none"><audio id="audio2"><source src="' + oxfordFlippedApp.sounds[2] + '" type="audio/mpeg"></audio></div>';
			customSound += '<div style="display: none"><audio id="audio3"><source src="' + oxfordFlippedApp.sounds[3] + '" type="audio/mpeg"></audio></div>';
...
oxfordFlippedApp.activityFinalScreenOne = function(contentZoneIndex) {
	...
	if (prevContentZone != '') {
		var customAnimation = '<div class="moneda-1 flip animated infinite"></div><div class="moneda-2 flip animated infinite"></div><div class="moneda-3 flip animated infinite"></div><div class="moneda-4 flip animated infinite"></div><div class="moneda-5 flip animated infinite"></div>',
				html = '<div class="oxfl-final-screen-one">'+customAnimation+'<div class="oxfl-final-screen-one-inner"><div class="oxfl-final-screen-one-bubble-1"> ' + oxfordFlippedApp.text.welldone + '</div> <div class="oxfl-coins-bubble-1"><div class="oxfl-coins-bubble-1-coins" id="oxfl-total-coins-1"></div></div><div class="oxfl-final-screen-one-bubble-2"> ' + oxfordFlippedApp.text.continue + ' </div></div></div>';
				...
				if (grade > oxfordFlippedApp.config.minGrade && grade != '') {
					...
					if (isChallenge) {
						var finalSlideBackground = $lastSlide.find('.image_slide').attr('src'),
								...
								finalSlideContent = '<div id="oxfl-final-slide">'+customAnimation+'<div class="oxfl-final-slide-stars" id="oxfl-final-slide-stars"></div><div class="oxfl-bubble-challenge-fail"><span> '+ oxfordFlippedApp.text.keeptrying +' </span></div><div class="oxfl-final-slide-fail-buttons"><button class="oxfl-button-bubble oxfl-button-bubble-2 oxfl-js-go-to-start">'+oxfordFlippedApp.text.tryagain+'</button><div class="oxfl-separate-text">'+oxfordFlippedApp.text.or+'</div><button class="oxfl-button-bubble oxfl-button-bubble-3 oxfl-js-close-iframe-inside">'+oxfordFlippedApp.text.exit+'</button></div></div>';
						if (finalSlideBackground != '') {
							$lastSlide.css('background-image', 'url('+finalSlideBackground+')').find('.slide_aux').hide();
						}
						$lastSlide.addClass('oxfl-final-slide-challenge');
...
oxfordFlippedApp.oxflModalDatepicker = function(endDate, idActivity) {
	...
	var $modal = $('#oxfl-modal-datepicker');
	if (endDate !== '') {
		$modal.addClass('oxfl-modal-datepicker-update');
	} else {
		$modal.removeClass('oxfl-modal-datepicker-update');
	}
	$modal.modal('show');
...
$(document).ready(function() {
	...
	blink.events.on('course:refresh', (function() {
		...
		if (oxfordFlippedApp.config.isStudent) {
			oxfordFlippedApp.loadNotifications(oxfordFlippedApp.bookData);
			oxfordFlippedApp.loadGradebook(false, true);
		}
