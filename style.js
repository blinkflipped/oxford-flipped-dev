...
oxfordFlippedApp.gradeToStars = function(grade) {
	...
	var totalStars = 0;
	if (grade > oxfordFlippedApp.config.twoStarsGradeMax && grade <= oxfordFlippedApp.config.oneStarGradeMax) {
		totalStars = 1;
...
oxfordFlippedApp.homepage = function(data) {
	...
	//oxfl-notifications"><div class="oxfl-notifications-badge">'+totalNotifications+'</div
...
oxfordFlippedApp.loadNotifications = function(data) {
	...
	if (totalNotif > 0) {
		...
		$('#oxfl-notification .oxfl-notifications-badge').text(totalNotif);
...
oxfordFlippedApp.loadMarketplaceList = function(data,type,itemperpage) {
		...
		$('body').removeClass('oxfl-body-marketplace');

		$resourceWrapper.imagesLoaded({background: 'div, a, span, button'}, function(){
			$('body').addClass('oxfl-body-marketplace-'+type);
			oxfordFlippedApp.config.currentPage = 'oxfl-body-marketplace-'+type;
			$('#oxfl-custom-background').addClass('active');
			...
			if (isFinalSlide) {
				...
				loadScript(urlSeguimiento, true, function() {
					...
					if (grade > oxfordFlippedApp.config.minGrade && grade != '') {
						if (isChallenge) {
							var finalSlideContent = '<div id="oxfl-final-slide"> <div class="oxfl-final-slide-stars" id="oxfl-final-slide-stars"></div><div class="oxfl-coins-bubble-2"><div class="oxfl-coins-bubble-2-coins" id="oxfl-total-coins-2"></div></div><button class="oxfl-button-bubble oxfl-button-bubble-3 oxfl-js-close-iframe-inside">'+oxfordFlippedApp.text.exit+'</button></div>';
